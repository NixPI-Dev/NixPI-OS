...
vi.mock("../extension/paths.js", () => ({
  ...
  isWikiPagePath: (wikiRoot: string, pathValue: string) => state.pagePath && pathValue.startsWith(`${wikiRoot}/pages/`),
}));
...
describe("nixpi-wiki extension wiring", () => {
  const originalHome = process.env.HOME;

  beforeEach(() => {
    state.wikiRoot = path.join("/tmp", "nixpi-wiki-index-test");
    state.technicalRoot = path.join("/tmp", "nixpi-wiki-index-test-technical");
    state.personalRoot = path.join("/tmp", "nixpi-wiki-index-test-personal");
    ...
    process.env.HOME = path.join("/tmp", "nixpi-wiki-home-test");
  ...
  it("registers all nixpi-wiki tools", async () => {
    const api = await loadExtension();
  ...
  it("rebuilds every edited split wiki root on agent_end", async () => {
    state.pagePath = true;
    const api = await loadExtension();
    await api.fireEvent("tool_call", {
      toolName: "edit",
      input: { path: `${state.technicalRoot}/pages/resources/technical/foo.md` },
    });
    await api.fireEvent("tool_call", {
      toolName: "write",
      input: { path: `${state.personalRoot}/pages/resources/personal/bar.md` },
    });
    await api.fireEvent("agent_end");

    expect(state.rebuildCalls).toEqual([state.technicalRoot, state.personalRoot]);
  });
