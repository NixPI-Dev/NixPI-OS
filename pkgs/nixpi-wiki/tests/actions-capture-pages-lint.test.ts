...
describe("capture, pages, and lint", () => {
  ...
  beforeEach(() => {
    wikiRoot = mkdtempSync(path.join(os.tmpdir(), "nixpi-wiki-actions-"));
  ...
  it("captureText creates deterministic packets, manifests, and source pages", () => {
    ...
    if (result.isOk()) {
      ...
      expect(existsSync(pagePath)).toBe(true);
	      ...
	      expect(page).toContain("id: source/src-2026-04-19-002");
	      expect(page).toContain("schema_version: 1");
	      expect(page).toContain("object_type: source");
	      expect(page).toContain("created: 2026-04-19");
	      expect(page).toContain("updated: 2026-04-19");
